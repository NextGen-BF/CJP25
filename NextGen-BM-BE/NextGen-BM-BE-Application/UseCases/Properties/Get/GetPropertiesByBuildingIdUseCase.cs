...
namespace NextGen_BM_BE_Application.UseCases.Expenses.Get
{
    ...
    public sealed class GetPropertiesByBuildingIdUseCase(IPropertyRepository _propertyRepositories)
    {
        ...
        public async Task<IList<Property>> Execute(int buildingId, int? page, int? pageSize)
        {
            var result = await propertyRepository.GetPropertiesByBuildingIdAsync(buildingId, page, pageSize);
            return result;
        }
    }
}